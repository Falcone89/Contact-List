...
interface Props {
  contact: Contact
  onClose: () => void
  onSave: (id: number, data: Omit<Contact, 'id'>) => void
}

export function EditContactModal({ contact, onClose, onSave }: Props) {
  const [name, setName] = useState(contact.name)
  const [phone, setPhone] = useState(contact.phone)
  const [email, setEmail] = useState(contact.email)
  const [picture, setPicture] = useState<string | null>(contact.picture)
  ...
  const handleSubmit = () => {
    ...
    onSave(contact.id, {
      ...
      call_handling_enabled: contact.call_handling_enabled,
    })
    onClose()
  }
